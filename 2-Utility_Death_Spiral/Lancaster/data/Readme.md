...
(2) **Hourly Solar Output.xlsx** gives the solar output of a solar PV installation in the Boston area in watts on an hourly basis for a representative year. There are five tabs, for 2kW, 4kW, 6kW, 25kW and 75kW installations and there are 4 orientations (South, SouthWest, West, SouthEast)
...
(3) **roofCategory.xlsx** specifies 7 categories of building, based on roof size. We ID these as types 0, 1, 2, 3, 4, 5, and 6.
...
(6) **buildings_data.csv** was derived from the **GIS** shapefile. It is a CSV file with two columns per row and 51540 rows, one for each building. The first value in a row is the building ID and the second value is the building’s roof area in square meters.
