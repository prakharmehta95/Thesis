...
import solar_pv_with_adjusting_price as solar_pv
runData = pickle.load(open('pickles/buildingData.p','rb'))
Run=0
for initial_price in electricity_price_list:
    for neighborhood_effect in neighborhood_effect_list:
        for solar_PV_cost in solar_PV_cost_list:
             for loop in range(RunsPerScenario):   #To perform multiple runs for each scenario        
                ...
                (per_yr_stat,installed_solar_building_Type_dic, prosumers_updated, RRFC)=solar_pv.doit(
                       ticks_to_run, prosumers, runData, initial_price, F0, solar_PV_cost, cap_fac, neighborhood_effect, k, l_scale)
...
#%%
    ...
    #To create the first worksheet "Summary"
                ...
                parameters =['Ticks to run', 'Initial price', 'Solar PV cost', 'Capacity factor', 'neighborhood effect', 'k', 'L-scale']
                parameter_values =[ticks_to_run, initial_price, solar_PV_cost, cap_fac, neighborhood_effect, k, l_scale]
    ...
    #to creat the second sheet "Total"
                ...
                colnames=["Year","Adjusted Price", "Installed Units","New Installations", "Installed Capacity",
    ...
    #to report per year statistics with a new sheet for every year
                ...
                i=0 
                for category in solar_pv_init.buildingCategories: 
                    buildTypes[i]=category.encode('ascii','ignore'); i+=1 
    ...
    #the folowing variables are used to generate the second Excel file
                parameter_2_values =[Run, initial_price, per_yr_stat[-1][1], solar_PV_cost, cap_fac, neighborhood_effect]+per_yr_stat[-1][2:7:2]+per_yr_stat[-1][7:]
                if Run==1: 
                    parameters_2 =[['Run', 'Initial price', 'last year price', 'Solar PV cost', 'Capacity factor', 'neighborhood effect', 
